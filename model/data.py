...
class BaseDataset(Dataset):
    ...
    # def get_prefix_allowed_tokens_fn(self, tokenizer, trie):
    #     self.trie = trie
    #     sep = tokenizer(":")["input_ids"][1:][0]
    #
    #     def prefix_allowed_tokens_fn(batch_id, sentence):
    #         sequence = sentence.tolist()
    #         if sep in sequence:
    #             index_from_end = len(sequence) - 1 - sequence[::-1].index(sep)
    #             sub_sequence = sequence[index_from_end + 1:]
    #             cur = self.trie
    #             for s in sub_sequence:
    #                 cur = cur.children[s]
    #
    #             next_states = cur.children if cur else {}
...
class SeqRecDataset(BaseDataset):
    ...
    def _remap_items(self):
        ...
        self.remapped_inters2 = dict()
        for uid, items in self.inters.items():
            new_items = ["".join(self.indices[str(i)]) for i in items]
            self.remapped_inters[uid] = new_items
            self.remapped_inters2[uid] = [self.indices[str(i)] for i in items]

    def random_permutation(self, strings):
        results = [s + ''.join(random.choice(list(itertools.permutations([si for si in strings if si != s])))) for s in
                   strings]
        # for s in strings:
        #     l = [si for si in strings if si != s]
        #     all_permutations = list(itertools.permutations(l))
        #     results.append(s + ''.join(random.choice(all_permutations)))
        return results
    ...
    def _process_train_data(self):
        ...
        for uid  in self.remapped_inters:
            ...
            # split_items = self.remapped_inters2[uid][:-2]
            # # custom Wang
            # for i in range(1, len(split_items)):
            #     history = split_items[:i]
            #     one_sample = split_items[i]
            #     new_sample = {}
            #     # custom Wang
            #     for j in range(4):
            #         new_sample["inters"] = self.his_sep.join(''.join(h[:j + 1]) for h in history)
            #         new_sample["item"] = ''.join(one_sample[:j + 1])
            #         inter_data.append(new_sample.copy())
            ...
            #     # custom Wang
            #
            #     permutations = self.random_permutation(one_sample)
            #     inters = self.his_sep.join(''.join(h) for h in history)
            #     for j, permutation in enumerate(permutations):
            #         new_sample["inters"] = inters
            #         new_sample["item"] = permutation
            #         inter_data.append(new_sample.copy())
            ...
            #     new_sample = {}
            #
            #     permutation = self.random_sampling_one_permutation(one_sample)
            #     inters = self.his_sep.join(''.join(h) for h in history)
            #     new_sample["inters"] = inters
            #     new_sample["item"] = permutation
            #     inter_data.append(new_sample)
    ...
    def _process_valid_data(self):
        ...
        for uid in self.remapped_inters:
            ...
            # split_items = self.remapped_inters2[uid]
            # history = split_items[:-2]
            # one_sample = split_items[-2]
            # new_sample = {}
            # # custom Wang
            # for i in range(4):
            #     new_sample["inters"] = self.his_sep.join(''.join(h[:i+1]) for h in history)
            #     new_sample["item"] = ''.join(one_sample[:i+1])
            #     inter_data.append(new_sample.copy())
...
class FusionSeqRecDataset(BaseDataset):
    ...
    def _get_interaction_graph_from_train_data(self):
        return
